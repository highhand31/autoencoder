...
class AE():
    ...
    def __build_model(self):
        ...
        self.ori_x = tf.placeholder(tf.float32, self.input_dim, name="ori_x")

        self.prediction = self.__inference(self.noise_x)
    ...
    def train(self, origin_data,noise_data,test_data,test_label, GPU_ratio=0.2, epochs=50, batch_size=1, fine_tune=False,
              save_ckpt=True):
                ...
                for epoch in range(epochs):
                    ...
                    for index in range(total_batches):
                        ...
                        sess.run(self.optimizer, feed_dict={self.ori_x: origin_data[num_start:num_end],
                                                            self.noise_x:noise_data[num_start:num_end]})
                    ...
                    for index in range(origin_data.shape[0]):
                        single_loss = sess.run(self.loss, feed_dict={self.ori_x: origin_data[index:index+1],
                                                                     self.noise_x:origin_data[index:index+1]})
...
if __name__ == "__main__":
    ...
    # ae = AE(input_dim=[None, width, height, 3], save_path="model_saver\ckpt")
    # ae.train(x_train,x_test,x_test_label,epochs = 50,fine_tune=False)
    # print("AI訓練結束")