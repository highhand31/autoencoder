...
class AE():
    ...
    def __build_model(self):
        ...
        self.input_center = tf.placeholder(tf.float32,[None,784], name="input_center")
        ...
        encode_shape = self.encode2.shape
        self.input_x_flatten = tf.reshape(self.encode2, (-1, encode_shape[1]*encode_shape[2]*encode_shape[3]))
        self.loss_AE = tf.reduce_mean(tf.pow(self.prediction - self.input_x, 2), name="loss_AE")
        # self.loss_kmeans = tf.reduce_mean(self.input_center*tf.pow(tf.log(self.input_x_flatten+1) - tf.log(self.input_center+1), 2), name="loss_kmeans")
        ...
        # self.loss_kmeans = tf.reduce_mean(self.input_center*()tf.log(self.input_center / self.input_x_flatten),name="loss_kmeans")
    ...
    def __inference(self, input_x):
        ...
        # -----------------------------------------------------------------------
        # data= 7 x 7 x 16
        encode2_shape = self.encode2.shape
        ...
        net = tf.layers.dense(inputs=net, units=encode2_shape[1] * encode2_shape[2] * encode2_shape[3],
                              activation=tf.nn.relu, name='FC3')
        # data= 784
    ...
    def train_AE_kmeans(self, origin_data,label_data, GPU_ratio=0.2, epochs=50, batch_size=1, fine_tune=False,
        ...
        with tf.Session(config=config) as sess:
            ...
            if fine_tune is True:
                ...
                model_filename = r"model_saver\AE_FC_mnist\pb_model.pb"
            ...
            kmeans = tf.contrib.factorization.KMeansClustering(
                num_clusters=10, use_mini_batch=True, model_dir=None,
                initial_clusters=KMeansClustering.KMEANS_PLUS_PLUS_INIT)
            for index in range(self.k_num_iterations):
            ...
            for epoch in range(epochs):
                ...
                output_graph_def = graph_util.convert_variables_to_constants(sess, graph,
                                                                             ['output/Relu', 'loss',
                                                                              "pool2","output_FC"])  # graph也可以直接填入sess.graph_def
                addr_tail = "pb_model.pb"
                pb_addr = os.path.join(self.save_path, addr_tail)
                with tf.gfile.GFile(pb_addr, "wb") as f:
                    f.write(output_graph_def.SerializeToString())
...
if __name__ == "__main__":
    save_path = r"model_saver\AE_FC_Kel9_Kmeans"
    out_dir_prefix = os.path.join(save_path, "model")
    height = 28
    width = 28
    epochs = 13
    ...
    # E:\dataset\Surface_detection

    ae = AE(input_dim=[None, width, height,1], save_path=save_path)
    #temp = ae.get_encode_init_data(input_mnist)
    #print(temp.shape)
    ae.train_AE_kmeans(input_mnist,label_mnist, GPU_ratio=GPU_ratio, epochs=epochs, fine_tune=True)
    # print("AI訓練結束")